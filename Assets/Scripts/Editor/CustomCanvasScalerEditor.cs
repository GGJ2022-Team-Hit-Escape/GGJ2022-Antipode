...
namespace UnityEditor.UI
{
    ...
    /// </summary>
    public class CustomCanvasScalerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                showWorld = ((target as CustomCanvasScaler).GetComponent<Canvas>().renderMode == RenderMode.WorldSpace);
                ...
                {
                    CustomCanvasScaler scaler = targets[i] as CustomCanvasScaler;
            ...
            if (!showWorldDiffers && !(!showWorld && m_UiScaleMode.hasMultipleDifferentValues))
            {
                ...
                // Constant pixel size
                else if (m_UiScaleMode.enumValueIndex == (int)CustomCanvasScaler.ScaleMode.ConstantPixelSize)
                {
                    EditorGUILayout.PropertyField(m_ScaleFactor);
                }
                // Scale with screen size
                else if (m_UiScaleMode.enumValueIndex == (int)CustomCanvasScaler.ScaleMode.ScaleWithScreenSize)
                {
                    EditorGUILayout.PropertyField(m_ReferenceResolution);
                    EditorGUILayout.PropertyField(m_ScreenMatchMode);
                    if (m_ScreenMatchMode.enumValueIndex == (int)CustomCanvasScaler.ScreenMatchMode.MatchWidthOrHeight && !m_ScreenMatchMode.hasMultipleDifferentValues)
                    {
                ...
                else if (m_UiScaleMode.enumValueIndex == (int)CustomCanvasScaler.ScaleMode.ConstantPhysicalSize)
                {
